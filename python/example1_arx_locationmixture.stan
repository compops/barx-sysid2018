...
parameters {
  ...
  real<lower=0, upper=10> observationNoiseVariance;
...
model {
  ...
  for (k in 1:noComponents)
        ...
        //mixtureWeightsPriorVector[k] = 1.0;
  ...
  for (n in 1:noEstimationData) {
    for (k in 1:noComponents)
        tmp[k] = log(mixtureWeights[k]) + normal_lpdf(yEstimation[n] | mixtureMeans[k] + regressorMatrixEstimation[n, :] * modelCoefficients, observationNoiseVariance);
...
generated quantities {
    ...
    for (n in 1:noGridPoints) 
        mixtureOnGrid[n] = exp(normal_lpdf(gridPoints[n] | mixtureMeans[classIndex], observationNoiseVariance));

    for (n in 1:noValidationData) {
        predictiveMean[n] = mixtureMeans[classIndex] + regressorMatrixValidation[n, :] * modelCoefficients;
        predictiveVariance[n] = observationNoiseVariance;
    }
}
